			...
			for k in pairs(locations) do
				if GetDistanceBetweenCoords(locations[k].x, locations[k].y, locations[k].z, GetEntityCoords(GetPlayerPed(-1))) < 150 then			
					...
					if GetDistanceBetweenCoords(locations[k].x, locations[k].y, locations[k].z, GetEntityCoords(GetPlayerPed(-1)), false) < 2 then			
						TriggerServerEvent('KMeth:get')
						TriggerEvent('KMeth:new', k)
					end
...
local making = true
	...
	ESX.TriggerServerCallback('KMeth:process', function(output)
			making = output
		end)
...
RegisterNetEvent('KMeth:start')
AddEventHandler('KMeth:start', function()
	local set = false
	Citizen.Wait(10)
...
RegisterNetEvent('KMeth:new')
AddEventHandler('KMeth:new', function(id)
	local set = false
	Citizen.Wait(10)
...
RegisterNetEvent('KMeth:message')
AddEventHandler('KMeth:message', function(message)
